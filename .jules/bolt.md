...
## 2025-07-04 - Eliminate dynamic allocation in FEN generation
**Learning:** Functions like `std::to_string` require a dynamic heap allocation which is very expensive when performed repeatedly in a hot path, such as during the generation of FEN strings from positions (where the integer values for things like half-move clock are frequently single digits).
**Action:** Prefer `static_cast<char>('0' + num)` for small single-digit integers rather than allocating a full `std::string` using `std::to_string`.
